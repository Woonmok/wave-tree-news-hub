...
chmod +x "$PUBLISH_SCRIPT_PATH" "$DAILY_SCRIPT_PATH" "$HEALTHCHECK_SCRIPT_PATH" "$RECONCILE_SCRIPT_PATH" "$MISSED_GUARD_SCRIPT_PATH"
...
if [ -f "$ANTIGRAVITY_SCRIPT_PATH" ]; then
  echo "- 명령6: $ANTIGRAVITY_CRON_CMD"
else
  echo "- 명령6: (건너뜀) antigravity watchdog 스크립트 없음: $ANTIGRAVITY_SCRIPT_PATH"
fi
echo ""
echo "📋 현재 crontab"
crontab -l

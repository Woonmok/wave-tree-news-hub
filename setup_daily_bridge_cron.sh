...
chmod +x "$PUBLISH_SCRIPT_PATH" "$DAILY_SCRIPT_PATH" "$HEALTHCHECK_SCRIPT_PATH"
[ -f "$ANTIGRAVITY_SCRIPT_PATH" ] && chmod +x "$ANTIGRAVITY_SCRIPT_PATH"

CURRENT_CRON=$(crontab -l 2>/dev/null || true)
FILTERED=$(printf "%s\n" "$CURRENT_CRON" | grep -F -v "$PUBLISH_SCRIPT_PATH" | grep -F -v "$DAILY_SCRIPT_PATH" | grep -F -v "$HEALTHCHECK_SCRIPT_PATH" | grep -F -v "$ANTIGRAVITY_SCRIPT_PATH" || true)
...
echo "✅ cron 등록 완료"
echo "- 스케줄: 매일 06:50 (publish), 07:00 (daily bridge), 07:05 (healthcheck), 2분 주기 (antigravity watchdog)"
echo "- 명령1: $PUBLISH_CRON_CMD"
echo "- 명령2: $DAILY_CRON_CMD"
echo "- 명령3: $HEALTHCHECK_CRON_CMD"
if [ -f "$ANTIGRAVITY_SCRIPT_PATH" ]; then
  echo "- 명령4: $ANTIGRAVITY_CRON_CMD"
else
  echo "- 명령4: (건너뜀) antigravity watchdog 스크립트 없음: $ANTIGRAVITY_SCRIPT_PATH"
fi
echo ""
echo "📋 현재 crontab"
crontab -l
